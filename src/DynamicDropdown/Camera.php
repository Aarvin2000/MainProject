 ...
 */
class Camera implements SelectOptionsProviderInterface
{
    ...
    {
        $items = new DataObject\Camera\Listing();
        $items->setOrderKey("megaPixel");
        $items->setOrder('asc');
        $arr = [];

        foreach ($items as $item) {
            array_push($arr, ["value" => $item->getMegaPixel(), "key" => $item->getMegaPixel()]);
        }
